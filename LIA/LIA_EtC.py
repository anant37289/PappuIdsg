...
from src.EtC.EtC_cifar import EtC_cifar8
...
def main():
    ...
    for i in range(epoch):
        if dataset == "MNIST":
            C = ConvNet(width=8, num_channels=1, num_classes=10).to(
                device
            )  # change the channel if you need
            ...
            C = ConvNet(width=8, num_channels=3, num_classes=10).to(
                device
            )  # change the channel if you need
