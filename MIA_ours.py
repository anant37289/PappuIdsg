...
def attack_inference(model, test_X, test_Y, device):
    ...
    dataloader = torch.utils.data.DataLoader(
        dataset=inferset, batch_size=50, shuffle=False, num_workers=0
    )
...
def get_data_loader(dataset="mnist", batch=64, shadow_split=0.5, num_workers=0):
    ...
    if dataset == "mnist":
        ...
        test_set = torchvision.datasets.SVHN(
            root="./data/svhn/", split="test", download=False, transform=test_transforms
        )
    elif dataset == "fmnist":
...
def create_attack(num, dataset, victim, others):
    ...
    t_train_loader = DataLoader(
        DatasetSplit(dataset, idxs_train), batch_size=64, shuffle=False
    )
    idxs_test = others[: int(1 * len(others))]
    t_test_loader = DataLoader(
        DatasetSplit(dataset, idxs_test), batch_size=64, shuffle=False
    )

    # target madel
    if args.dataset == "mnist":
        G = Generator(1, 32, 1, 6).to(
            device
        )  # cifar10, svhn: channel(3); mnist, fmnist: channel(1)
        G_O = Generator(1, 32, 1, 6).to(
            device
        )  # cifar10, svhn: channel(3); mnist, fmnist: channel(1)
        C = VGG16_classifier_mnist().to(device)
        Fea = AutoEncoder_VGG_mnist().to(
            device
        )  # cifar10, svhn: AutoEncoder_VGG; mnist, fmnist: AutoEncoder_VGG_mnist
        F_O = AutoEncoder_VGG_mnist().to(
            device
        )  # cifar10, svhn: AutoEncoder_VGG; mnist, fmnist: AutoEncoder_VGG_mnist
    ...
    Fea.load_state_dict(
        torch.load(modelPath + "0_extractor_param.pkl")
    )  # victim feature extractor
    if num == 1:
        G_O.load_state_dict(
            torch.load(modelPath + "1_generator_param.pkl")
        )  # other generator (part situation)
        F_O.load_state_dict(
            torch.load(modelPath + "1_extractor_param.pkl")
        )  # other feature extractor
    else:
        G_O.load_state_dict(
            torch.load(modelPath + "others_generator_param.pkl")
        )  # others generator
        F_O.load_state_dict(
            torch.load(modelPath + "others_extractor_param.pkl")
        )  # others feature extractor (all situation)

    # the skeptical dataset
    t_trainX, t_trainY = prepare_target_data(
        G, Fea, C, t_train_loader, device, victim=True
    )
    t_testX, t_testY = prepare_target_data(
        G_O, F_O, C, t_test_loader, device, victim=False
    )
    ...
    if args.dataset == "mnist" or args.dataset == "fmnist":
        F_ini = AutoEncoder_VGG_mnist().to(
            device
        )  # attacker cannot get the feature extractor
    else:
        F_ini = AutoEncoder_VGG().to(device)
    print("----Preparing Attack training data---")
    trainX, trainY = prepare_target_data(
        G, F_ini, C, s_train_loader, device, victim=True
    )
    testX, testY = prepare_target_data(
        G_O, F_ini, C, s_test_loader, device, victim=False
    )
    ...
    attack_model = AttackMLP(
        input_size=input_size, hidden_size=n_hidden, out_classes=2
    ).to(device)

    # Loss and optimizer
    attack_loss = nn.CrossEntropyLoss()
    attack_optimizer = torch.optim.Adam(
        attack_model.parameters(), lr=0.005, weight_decay=1e-4
    )
    attack_lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(
        attack_optimizer, gamma=0.96
    )
    ...
    attack_valacc = train_attack_model(
        attack_model,
        attackdataset,
        attack_loss,
        attack_optimizer,
    ...
    print(
        "Validation Accuracy for the Best Attack Model is: {:.2f} %".format(
            100 * attack_valacc
        )
    )
...
if __name__ == "__main__":
    ...
    for epoch in tqdm(range(args.num_epochs)):
        ...
        if test_acc > best_test_acc:
            ...
            for i in range(len(sum_weights1)):
                torch.save(
                    sum_weights1[i], model_dir + str(i) + "_generator_param.pkl"
                )  # victim generator
                torch.save(
                    sum_weights2[i], model_dir + str(i) + "_extractor_param.pkl"
                )  # victim feature extractor
                torch.save(
                    sum_weights3[i], model_dir + str(i) + "_classifier_param.pkl"
                )  # victim classifier
    ...
    create_attack(
        num=1,
        dataset=train_dataset,
        victim=list(user_groups[0]),
        others=list(user_groups[1]),
    )
    print("------num=10---------")  # all situation
    create_attack(
        num=10, dataset=train_dataset, victim=list(user_groups[0]), others=others
    )
