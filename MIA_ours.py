...
def get_data_loader(dataset='mnist', batch=64, shadow_split=0.5, num_workers=0):
    ...
    if dataset == 'mnist':
        ...
        test_set = torchvision.datasets.CIFAR10(root='./data/cifar/', train=False, download=True,
                                                transform=test_transforms)##this is now cifar100
    elif dataset == 'svhn':
        test_set = torchvision.datasets.SVHN(root='./data/svhn/', split='test', download=True,
                                             transform=test_transforms)
    elif dataset == 'fmnist':
        test_set = torchvision.datasets.FashionMNIST(root='./data/fmnist/', train=False, download=True,
                                                     transform=test_transforms)
