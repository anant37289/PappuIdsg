...
class LocalUpdate(object):
    ...
    def train_val_test(self, dataset, idxs):
        ...
        trainloader = DataLoader(
            DatasetSplit(dataset, idxs_train), batch_size=64, shuffle=True
        )
        return trainloader
    ...
    def update_weights(self, net, global_round):
        ...
        if (global_round + 1) > self.args.decay_epoch:
            optimizer.param_groups[0]["lr"] -= self.args.lr / (
                self.args.epochs - self.args.decay_epoch
            )
        ...
        for iter in range(self.args.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                ...
                if self.args.dataset == "fmnist" or self.args.dataset == "mnist":
                    image = EtC_mnist(imgs).to(
                        self.device
                    )  # mnist, fmnist: EtC_mnist; cifar10, svhn: EtC_cifar
                else:
                    image = EtC_cifar8(imgs).to(self.device)
